...
def calculate_cum_features_old(row: pd.Series, data: pd.DataFrame, n_years: int) -> pd.Series:
    ...
    data['days_since'] = (end_date - data['STARTDATEPLAN']).dt.days

    # Set decay rate
    lambda_decay = 0.01

    # Calculate weights using exponential decay
    data['weights'] = np.exp(-lambda_decay * data['days_since'])

    # Calculate weighted no-show percentage
    weighted_no_show_percentage = np.sum(data['no_show'] * data['weights']) / np.sum(data['weights'])
